...
public class TollPlaza {
	...
	//check that fastag & number plate is matching with database.
	public static boolean isfastagMatch(int fastagId, String licencePlateId) {

		boolean isMatched = false;

		for (Fastag tag : VehicleDatabase.fastags) {
			if ((tag.getFastagId() == fastagId)
					&& (tag.getVehicle().getLicencePlateId().toLowerCase().equals(licencePlateId.toLowerCase()))) {
				isMatched = true;
	...
	public static void main(String[] args) throws InsuffcientFundsException {
		...
		while (true) {
			...
			if (isfastagMatch(fastagId, licencePlateId)) {
				...
				System.out.println(
						"Your entered fastag id or licence plate id is not matching with our database.\nTry Again!");
			}
		}
		...
		//loop will go till toll amount will successful deduct.
		while (true) {
			...
			if (wantToRecharge.equals("yes")) {
				...
				int amountToBeRecharged;
				//loop goes until user enter valid input.
				while (true) {
					
					while (!sc.hasNextInt()) sc.next(); // this loop will go on till user input integer
					amountToBeRecharged = sc.nextInt();
					
					if (amountToBeRecharged > 0)
						break;
					else {
						System.out.print("Amount should be greater than 0 only, Enter amount again: ");
						continue;
					}
				...
				if (paymentMethod.equals("card")) {
					...
					while (true) {
						...
						if (msgFromValidationCard.equals("valid")) {
							...
							if (cardNumber.equals(VehicleDatabase.fastags.get(fasTagListIndex).getBankAccount()
									.getDebitCardNumber())) {

								String msgFromRechargeFastag = RechargeFastag.rechargefastag(
										VehicleDatabase.fastags.get(fasTagListIndex), amountToBeRecharged);
					...
					while (true) {
						...
						if (msgFromValidationUpi.equals("valid")) {
							...
							if (upiId
									.equals(VehicleDatabase.fastags.get(fasTagListIndex).getBankAccount().getUpiId())) {
								...
								System.out.println(
										"Your upiId is not matching with your linked bank account.Try again!\n");
								continue;
							}
					...
					System.out.println("Please enter only card or upi.");
				}
				...
				if (FastagMinBalance.hasMinimumBalance(VehicleDatabase.fastags.get(fasTagListIndex))) {
					...
					System.out.println(
							"Currunt balance is : " + VehicleDatabase.fastags.get(fasTagListIndex).getFastagBalance());
					System.out.println("Minimum Required balance is : "
							+ FastagMinBalance.getMinBalance(VehicleDatabase.fastags.get(fasTagListIndex)));
